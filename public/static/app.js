...
(function () {
  ...
  async function submitContact(e) {
    ...
    try {
      ...
      if (res.success) {
        msgEl.style.color = '#2f7a4f';
        msgEl.textContent = res.message || 'Message sent!';
        e.target.reset();
      } else {
        msgEl.style.color = '#b23b3b';
        msgEl.textContent = res.error || 'Failed to send message.';
      }
    } catch (err) {
      msgEl.style.color = '#b23b3b';
      msgEl.textContent = 'Something went wrong. Please try again.';
  ...
  async function loadSettings() {
    try {
      ...
      setContact('contactEmailRow', '<i class="fa-solid fa-envelope"></i>', s.email);
